...
def test_single(cosyvoice: CosyVoice2, text, spk, outfile):
    ...
    for i, j in enumerate(cosyvoice.inference_sft_nopitch(text, spk_name=spk, source_speech_token=source_speech_token, stream=False, text_frontend=False, lang='hindi')):
        audio_gen = j['tts_speech']
        torchaudio.save(outfile, audio_gen, cosyvoice.sample_rate)
...
def test_single_instruct(cosyvoice: CosyVoice2, text, prompt_audio_16k, outfile):
    ...
    for i, j in enumerate(cosyvoice.inference_instruct2(text, 'खुशी', prompt_speech_16k, stream=False, text_frontend=False, lang='hindi')):
...
def main():
    parser = argparse.ArgumentParser(description="CosyVoice2 inference script")
    parser.add_argument("-m", "--model_path", type=str, required=True, help="Path to trained CosyVoice2 model, for example: trained_models/bbc07230902_yoyo0904_thres300")
    parser.add_argument("--spk_name", type=str, default="37863166", help="Speaker name")
    # parser.add_argument("-t", "--test_file", type=str, default="datas/tttest/test_2spk.jsonl", help="Test JSONL file")
    parser.add_argument("-t", "--test_file", type=str, 
                        default="/data/liangyunming/tts_20250618/CosyVoice/examples/hindi/cosyvoice2/others/out_spk_ref/test100.jsonl", 
                        help="Test JSONL file")
    parser.add_argument("-o", "--out_dir", type=str, required=True, help="Output directory")
    parser.add_argument("--speech_token_file", type=str, default=None, help="Optional speech token file")
    args = parser.parse_args()

    # 初始化
    cosyvoice, sr = init_cosyv2(args.model_path, spk_name=args.spk_name,)
    print(f"✅ CosyVoice2 initialized with sample rate: {sr}")

    # 推理
    test_cosyvoice2_nopitch_infer(
        cosyvoice,
        sr,
        args.test_file,
        args.out_dir, 
        args.speech_token_file,
    )
