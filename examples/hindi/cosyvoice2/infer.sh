...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
  ...
  for num in 5; do
    ...
    CUDA_VISIBLE_DEVICES=0 python cosyvoice/bin/inference.py --mode sft \
      ...
      --flow_model exps/v1_1000h-bbc_v1_240/cosyvoice/flow/torch_ddp/epoch_0_whole_infer.pt \
      --hifigan_model $pretrained_model_dir/hift.pt \
      --result_dir output/output_${exp_name}_sss_flow0/avg_3_5 \
      --epoch avg${num}
  done
fi
