...
def get_sample(idx, username):
    ...
    if 0 <= idx < total_num:
        ...
        return (
            wav_path,
            entry.get("sentence_type", "Declarative"),
            format_progress(idx)
        )
    else:
...
def save_and_next(sentence_type, idx, username):
    if idx >= total_num:
...
def save_and_prev(sentence_type, idx, username):
    if idx >= total_num:
        return None, None, "All audio files have been evaluated.", idx
...
def create_demo():
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                sentence_type = gr.Radio(
                    ...
                    label=(
                        "Choose one per clip based only on audio; if unsure, pick Other — this decides the sentence-ending punctuation.\n"
                        "Declarative ends with a vertical bar (।),"
                        "Interrogative ends with a question mark (?),"
                        "Exclamatory ends with an exclamation mark (!),"
                        "Other has no punctuation."
                    ),
                    value="Declarative"
                )
            with gr.Column():
        ...
        btn_confirm.click(
            ...
            audio_player, sentence_type, progress_text, idx_state,
            btn_next, btn_prev
        ])

        btn_prev.click(save_and_prev, inputs=[sentence_type, idx_state, name_input],
                    outputs=[audio_player, sentence_type, progress_text, idx_state])

        btn_next.click(save_and_next, inputs=[sentence_type, idx_state, name_input],
                    outputs=[audio_player, sentence_type, progress_text, idx_state])
