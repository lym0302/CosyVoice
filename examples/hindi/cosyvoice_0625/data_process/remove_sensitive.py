...
def process_data(input_file: str):
    ...
    return remove_sensitive(input_lines)
    
    
def remove_sensitive(input_lines):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="过滤包含敏感词的语音样本")
    parser.add_argument("-i", "--input_file", required=True, help="输入的 data.list 文件路径")
    parser.add_argument("-o", "--output_file", required=True, help="输出的 data_rmsenti.list 文件路径")
